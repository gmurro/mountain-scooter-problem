...
def main():
    ...
    velocity_state_array = np.linspace(-1.7, +1.7, num=tot_bins-1, endpoint=False)
    ...
    for episode in range(tot_episode):
        ...
        position, velocity = env.reset(exploring_starts=False)
        ...
        observation = (np.digitize(velocity, velocity_state_array),
                       np.digitize(position, position_state_array))
        is_starting = True
        cumulated_reward = 0
